...
async function findStartAndEnd(board) {
    ...
    for (let i = 0; i < board.length; i++) {
        ...
        visited[i] = [false, false, false, false, false, false, false, false, false, false];
    }
...
async function findWord(board, visited, word, r, c, idx) {
    ...
    if (word.charAt(idx) === board[r][c].textContent) {
        await sleep(value);
        board[r][c].style.backgroundColor = "green";
    }

    if (word.charAt(idx) !== board[r][c].textContent) {
        await sleep(value);
        if (board[r][c].style.backgroundColor !== "green") {
    ...
    let isWord =  await findWord(board, visited, word, r + 1, c, idx + 1) ||
